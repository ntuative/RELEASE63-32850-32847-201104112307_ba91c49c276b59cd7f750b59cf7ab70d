package com.sulake.habbo.sound.music
{
   ...
   public class HabboMusicController implements IHabboMusicController, IDisposable
   {
      ...
      private static const const_1345:int = HabboMusicPrioritiesEnum.const_122;
      ...
      public function HabboMusicController(param1:HabboSoundManagerFlash10, param2:IEventDispatcher, param3:IEventDispatcher, param4:IConnection)
      {
         ...
         this.var_719.push(new TraxSongInfoMessageEvent(this.onSongInfoMessage));
         this.var_719.push(new UserSongDisksInventoryMessageEvent(this.onSongDiskInventoryMessage));
         for each(_loc5_ in this.var_719)
         {
            this._connection.addMessageEvent(_loc5_);
         }
         this.var_82.addEventListener(RoomEngineSoundMachineEvent.const_828,this.onJukeboxInit);
         this.var_82.addEventListener(RoomEngineSoundMachineEvent.const_276,this.onJukeboxDispose);
         this.var_82.addEventListener(RoomEngineSoundMachineEvent.const_822,this.onSoundMachineInit);
         this.var_82.addEventListener(RoomEngineSoundMachineEvent.const_734,this.onSoundMachineDispose);
         this.var_1143 = new Timer(1000);
         this.var_1143.start();
         this.var_1143.addEventListener(TimerEvent.TIMER,this.sendNextSongRequestMessage);
         this._events.addEventListener(SoundCompleteEvent.TRAX_SONG_COMPLETE,this.onSongFinishedPlayingEvent);
         _loc6_ = 0;
         while(_loc6_ < HabboMusicPrioritiesEnum.const_226)
         {
            this.var_502[_loc6_] = null;
            this.var_741[_loc6_] = 0;
            _loc6_++;
         }
      }
      ...
      protected function onSongFinishedPlayingEvent(param1:SoundCompleteEvent) : void
      {
         ...
         {
            if(this.getTopRequestPriority() == this._priorityPlaying && this.getSongRequestCountAtPriority(this._priorityPlaying) == this.var_1728)
            {
               this.resetSongStartRequest(this._priorityPlaying);
            }
            _loc2_ = this._priorityPlaying;
            this.playSongWithHighestPriority();
            if(_loc2_ >= HabboMusicPrioritiesEnum.const_416)
            {
               this._events.dispatchEvent(new NowPlayingEvent(NowPlayingEvent.const_606,_loc2_,param1.id,-1));
            }
         }
      }
      ...
      public function dispose() : void
      {
         ...
         if(!this._disposed)
         {
            ...
            {
               _loc2_ = 0;
               while(_loc2_ < this.var_419.length)
               {
                  _loc3_ = this.var_419.getWithIndex(_loc2_) as SongDataEntry;
            ...
            if(this.var_82)
            {
               this.var_82.removeEventListener(RoomEngineSoundMachineEvent.const_828,this.onJukeboxInit);
               this.var_82.removeEventListener(RoomEngineSoundMachineEvent.const_276,this.onJukeboxDispose);
               this.var_82.removeEventListener(RoomEngineSoundMachineEvent.const_822,this.onSoundMachineInit);
               this.var_82.removeEventListener(RoomEngineSoundMachineEvent.const_734,this.onSoundMachineDispose);
            }
      ...
      private function getSongIdRequestedAtPriority(param1:int) : int
      {
         ...
         if(this.var_502[param1] == null)
         {
            return -1;
         }
         var _loc2_:SongStartRequestData = this.var_502[param1];
      ...
      private function getSongRequestCountAtPriority(param1:int) : int
      {
         ...
         return this.var_741[param1];
      }
      
      private function getTopRequestPriority() : int
      {
         var _loc1_:int = this.var_502.length - 1;
         while(_loc1_ >= 0)
         {
            if(this.var_502[_loc1_] != null)
            {
               return _loc1_;
            }
            _loc1_--;
      ...
      private function getSongDataEntry(param1:int) : SongDataEntry
      {
         ...
         {
            _loc2_ = this.var_419.getValue(param1) as SongDataEntry;
         }
         return _loc2_;
      }
      ...
      public function updateVolume(param1:Number) : void
      {
         ...
         var _loc4_:* = null;
         var _loc2_:int = 0;
         while(_loc2_ < HabboMusicPrioritiesEnum.const_226)
         {
      ...
      {
         if(this.var_612.getValue(param1) == null)
         {
            this.var_612.add(param1,param2);
      ...
      {
         if(param1 >= 0 && param1 < this.var_342.length)
         {
            return this.var_342.getKey(param1);
         }
         return -1;
      }
      ...
      public function getSongDiskInventorySongId(param1:int) : int
      {
         if(param1 >= 0 && param1 < this.var_342.length)
         {
            return this.var_342.getWithIndex(param1);
         }
         return -1;
      }
      ...
      private function sendNextSongRequestMessage(param1:TimerEvent) : void
      {
         ...
         this._connection.send(new GetSongInfoMessageComposer(this.var_740));
         Logger.log("Requested song info\'s : " + this.var_740);
         this.var_740 = new Array();
      }
      ...
      private function onSongInfoMessage(param1:IMessageEvent) : void
      {
         ...
         while(_loc5_ < _loc4_.length)
         {
            ...
            if(_loc7_)
            {
               ...
               this.var_419.add(_loc6_.id,_loc10_);
               ...
               this._events.dispatchEvent(new SongInfoReceivedEvent(SongInfoReceivedEvent.const_87,_loc6_.id));
               while(this.var_742.indexOf(_loc6_.id) != -1)
               {
                  this.var_742.splice(this.var_742.indexOf(_loc6_.id),1);
                  if(this.var_742.length == 0)
                  {
                     this._events.dispatchEvent(new SongDiskInventoryReceivedEvent(SongDiskInventoryReceivedEvent.const_491));
                  }
               }
      ...
      {
         if(param2 == -1 || param1 < 0 || param1 >= HabboMusicPrioritiesEnum.const_226)
         {
            return false;
         }
         ...
         this.var_1728 = this.getSongRequestCountAtPriority(param1);
         this.var_1727 = param2;
         if(this._priorityPlaying <= const_1345)
         {
            this.notifySongPlaying(_loc4_);
         }
         if(param1 > HabboMusicPrioritiesEnum.const_122)
         {
            this._events.dispatchEvent(new NowPlayingEvent(NowPlayingEvent.const_517,param1,_loc4_.id,-1));
         }
      ...
      private function notifySongPlaying(param1:SongDataEntry) : void
      {
         ...
         if(param1.length >= 8000 && (this.var_2355 != param1.id || _loc3_ > this.var_2354 + 8000))
         {
            this._soundManager.notifyPlayedSong(param1.name,param1.creator);
            this.var_2355 = param1.id;
            this.var_2354 = _loc3_;
         }
      }
      ...
      private function onSongDiskInventoryMessage(param1:IMessageEvent) : void
      {
         ...
         while(_loc4_ < _loc3_.songDiskCount)
         {
            ...
            if(this.var_419.getValue(_loc6_) == null)
            {
               this.var_742.push(_loc6_);
      ...
      private function onSoundMachineInit(param1:Event) : void
      {
         ...
         this.var_503 = new SoundMachinePlayListController(this._soundManager,this,this._events,this.var_82,this._connection) as IPlayListController;
      }
