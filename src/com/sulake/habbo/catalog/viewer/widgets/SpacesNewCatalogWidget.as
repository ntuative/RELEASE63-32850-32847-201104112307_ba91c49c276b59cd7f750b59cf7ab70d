package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class SpacesNewCatalogWidget extends ItemGridCatalogWidget implements IDisposable, ICatalogWidget, IItemGrid
   {
      ...
      public function SpacesNewCatalogWidget(param1:IWindowContainer)
      {
         ...
         {
            _loc2_ = 0;
            while(_loc2_ < this.var_447.numSelectables)
            {
               _loc3_ = this.var_447.getSelectableAt(_loc2_);
      ...
      override public function init() : Boolean
      {
         ...
         this.switchCategory(this.var_101[this.var_536]);
      ...
      {
         var _loc2_:int = this.var_818[this.var_536];
         var _loc3_:Offer = this.var_207[this.var_536][_loc2_];
      ...
      public function selectIndex(param1:int) : void
      {
         ...
         {
            _loc2_ = this.var_207[this.var_536][param1];
      ...
      override public function select(param1:IGridItem) : void
      {
         ...
         this.var_818[this.var_536] = (this.var_207[this.var_536] as Array).indexOf(_loc2_);
         this.updateRoomPreview();
      }
      
      private function updateRoomPreview() : void
      {
         var _loc1_:int = this.var_818[0];
         var _loc2_:int = this.var_818[1];
         var _loc3_:int = this.var_818[2];
      ...
      private function createOfferGroups() : Boolean
      {
         ...
         {
            this.var_174 = configurationAsset.content as XML;
            for each(offer in page.offers)
            {
               if(!(offer.pricingModel == Offer.const_411 || offer.pricingModel == Offer.const_365))
               {
                  continue;
               }
               ...
               if(!(product.productType == ProductTypeEnum.const_74 || product.productType == ProductTypeEnum.const_77))
               {
                  continue;
               }
               ...
               {
                  case "floor":
                     for each(pattern in this.var_174.floors.pattern)
                     {
                        for each(itemData in pattern.floor)
                        {
                           cloneOffer = offer.clone();
                           cloneOffer.extraParameter = itemData.@id;
                           (this.var_207[index] as Array).push(cloneOffer);
                        }
                     }
                     break;
                  case "wallpaper":
                     pattern = this.var_174.walls.pattern.(@id == offerPostfix)[0] as XML;
                     if(pattern)
                     {
                     ...
                     break;
                  case "landscape":
                     pattern = this.var_174.landscapes.pattern.(@id == offerPostfix)[0] as XML;
                     if(pattern)
                     {
      ...
      private function switchCategory(param1:String) : void
      {
         ...
         if(_loc2_ > -1)
         {
            ...
            page.replaceOffers(this.var_207[this.var_536],false);
            resetTimer();
            populateItemGrid();
            ...
            _loc3_ = this.var_818[this.var_536];
