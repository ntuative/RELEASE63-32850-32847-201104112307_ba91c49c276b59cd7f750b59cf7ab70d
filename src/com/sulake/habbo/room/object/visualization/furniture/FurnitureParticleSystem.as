package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_1482:ColorTransform;
      
      private var var_1917:Matrix;
      
      private var _translationMatrix:Matrix;
      
      private var var_728:Number = 1.0;
      ...
      public function FurnitureParticleSystem(param1:AnimatedFurnitureVisualization)
      {
         ...
         this.var_806.alphaMultiplier = 1;
         this.var_1482 = new ColorTransform();
         this.var_1917 = new Matrix();
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_97 = this.var_1212[param1];
         this.var_994 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_1916 >= 0)
         {
            this.var_263 = this._visualization.getSprite(this.var_1916);
            if(this.var_263 && this.var_263.asset)
            {
               ...
               if(this.var_87 == null)
               {
                  ...
                  if(this.var_806.alphaMultiplier != 1)
                  {
                     this.var_660 = new BitmapData(this.var_87.width,this.var_87.height,true,4278190080);
                  }
               }
               this.var_2340 = -this.var_263.offsetX;
               this.var_2341 = -this.var_263.offsetY;
            ...
            if(this.var_660)
            {
               this.var_660.fillRect(this.var_660.rect,4278190080);
            }
         }
      }
      ...
      public function getSpriteYOffset(param1:int, param2:int, param3:int) : int
      {
         ...
         {
            return this.var_97.y * this.var_1213;
         }
         return 0;
      }
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_994 && this.var_97.hasIgnited)
         {
            this.var_994 = true;
         }
         _loc8_ = this.var_1097 * this.var_1213;
         this.var_97.update();
         if(this.var_994)
         {
            if(this.var_97.roomObjectSpriteId >= 0)
            ...
            {
               this.var_87.draw(this.var_660,this.var_1917,this.var_806,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_97.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2340 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1213;
               _loc4_ = this.var_2341 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1213;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_1482.alphaMultiplier = _loc11_.alphaMultiplier;
                     this.var_87.draw(_loc7_,this._translationMatrix,this.var_1482,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2648 = parseInt(param1.@size);
         this.var_1916 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1097 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1213 = this.var_2648 / 64;
         this.var_728 = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this.var_728 = Math.min(this.var_728,1);
         this.var_806.alphaMultiplier = this.var_728;
         for each(_loc3_ in param1.emitter)
         {
            ...
            for each(_loc23_ in _loc3_.particles.particle)
            {
               ...
               _loc19_ = _loc23_.@is_emitter == "OfficialRoomListCtrl" ? false : true;
