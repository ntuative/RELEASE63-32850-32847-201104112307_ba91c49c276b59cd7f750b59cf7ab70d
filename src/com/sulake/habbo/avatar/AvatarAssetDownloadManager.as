package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_439:AssetLoaderStruct;
      
      private var var_987:Timer;
      ...
      private const const_2049:int = 100;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         if(_loc6_ == null)
         {
            this.var_439 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_987 = new Timer(this.const_2049,1);
         this.var_987.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_439)
         {
            this.var_439.addEventListener(AssetLoaderEvent.const_29,this.onConfigurationComplete);
            this.var_439.addEventListener(AssetLoaderEvent.const_48,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_439)
         {
            this.var_439.removeEventListener(AssetLoaderEvent.const_29,this.onConfigurationComplete);
            this.var_439.removeEventListener(AssetLoaderEvent.const_48,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         else
         {
            ...
            if(this.var_1471.indexOf("?") > 0)
            {
               _loc2_ = this.var_1471 + "&retry=" + this.var_985;
            }
            else
            {
               _loc2_ = this.var_1471 + "?retry=" + this.var_985;
            ...
            this.var_439 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2625,this._assets);
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1897)
         {
            this.var_1204.push([param1,param2]);
            return;
         }
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         while(_loc6_ < this.var_798.length)
         {
            _loc5_ = this.var_798[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_798.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_987.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_797.length > 0 && this.var_798.length < this.const_1654)
         {
            _loc1_ = this.var_797[0];
            _loc1_.startDownloading();
            this.var_798.push(this.var_797.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_797.indexOf(param1) == -1 && this.var_798.indexOf(param1) == -1)
         {
