package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_1384:IWindow;
      
      private var var_776:IFigurePartSet;
      ...
      private const const_2039:int = 13421772;
      
      private const const_2038:int = 16777215;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         this.var_776 = param3;
         this._window = param1;
         this.var_1384 = this._window.findChildByTag("BG_COLOR");
         this._colors = param4;
         this.var_1832 = param5;
         if(param3 == null)
         {
            this.var_693 = new BitmapData(1,1,true,16777215);
         }
         if(param3 != null)
         {
            _loc7_ = param3.parts;
            ...
            {
               this.var_1830 = Math.max(this.var_1830,_loc6_.colorLayerIndex);
            }
         }
         this.var_428 = this.var_68.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_693 != null && !this.var_1832)
            {
               _loc3_ = this.var_693;
            }
            else
            ...
            if(this.var_776)
            {
               switch(this.var_776.clubLevel)
               {
                  ...
                  case HabboClubLevelEnum.const_47:
                     _loc2_.style = HabboIconType.const_649;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this.var_1384.color = this.const_2039;
         }
         else
         {
            this.var_1384.color = this.const_2038;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_428.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_428.isFigureReady(_loc1_))
         {
            this.var_428.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_352[_loc6_] + "_" + FigureData.const_138;
               _loc4_ = this.var_428.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_632 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1831)
               {
                  ...
                  this.var_1831 = _loc9_.content as BitmapData;
               }
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               if(this.var_1832 && _loc2_.colorLayerIndex > 0)
               {
