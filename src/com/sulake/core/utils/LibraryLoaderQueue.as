package com.sulake.core.utils
{
   ...
   public class LibraryLoaderQueue extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1654:int = 4;
      ...
      {
         return this.var_465.length + this.var_285.length;
      }
      ...
      private function next() : void
      {
         ...
         if(!disposed)
         {
            while(this.var_285.length < const_1654 && this.var_465.length > 0)
            {
               _loc1_ = this.var_465.shift();
      ...
      private function removeLoader(param1:LibraryLoader) : void
      {
         ...
         try
         {
            index = this.var_465.indexOf(loader);
            if(index > -1)
            {
               this.var_465.splice(index,1);
            }
      ...
      private function isUrlInQueue(param1:String, param2:Boolean = true) : Boolean
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_465)
            {
               if(param2)
               {
